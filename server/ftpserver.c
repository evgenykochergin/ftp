...
{	
	...
	return (ftpserver_check_user(user, pass));
}
...
{		
	...
	/* Authenticate user */
	if (ftpserver_login(sock_control) == 1){
	...
	while (1) {				
		/* Wait for command */
		int rc = ftpserver_recv_cmd(sock_control, cmd, arg);
		...
		if (rc == 200 ) {	
			...
			if ((sock_data = ftpserver_start_data_conn(sock_control)) < 0) {
				close(sock_control);
				exit(1); 
			}
			...
			if (strcmp(cmd, "LIST")==0){ /* Do list */
				ftpserver_list(sock_data, sock_control);
			} 
